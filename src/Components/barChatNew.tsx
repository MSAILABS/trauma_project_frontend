...
const des: { [key: string]: string[] } = {
	'Airway & Respiration': ['Endotracheal Intubation/Intubation', 'Mechanical Ventilation', 'RSI meds', 'intubation'],
...
const SingleSignalChart = ({ signalKey, chartData, index }: BarChartProps) => {
	...
	const metaKeys = Object.keys(chartData.meta).filter(k => k !== 'lsi_description' && k !== 'lsi_sampling_rate')
		...
		datasets: [
			{
				label: 'Meta Values',
				data: dataset,
				backgroundColor: dataset.map((val, i) => (i === maxIndex && val > 0.5 ? 'red' : 'cyan')),
				borderWidth: 2,
				borderColor: dataset.map((val, i) => (i === maxIndex && val > 0.5 ? 'red' : 'cyan')),
			},
		],
	}
		...
		<Bar
			...
			options={{
				...
				plugins: {
					...
					title: { display: true, text: 'LSI GROUPS' },
					datalabels: {
						display: true,
						color: 'white',
						anchor: 'end',
...
const MultiSignalCharts = ({ chartData }: { chartData: DataState }) => {
	...
	const signalKeys = Object.keys(chartData.signals).filter(k => !k.endsWith('_time'))
